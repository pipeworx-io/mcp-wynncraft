...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wynncraft');
}
...
const tools: McpToolExport['tools'] = [
  { name: 'player', description: 'Fetch a Wynncraft player\'s stats by UUID or username: playtime, XP, guild, ranking, veteran status, and per-class totals.', inputSchema: { type: 'object', properties: { uuid_or_username: { type: 'string' } }, required: ['uuid_or_username'] } },
  ...
  { name: 'guild', description: 'Fetch Wynncraft guild details by name or prefix (set by=\'prefix\' to look up by tag); returns member list, level, XP, territories, and seasonal rating.', inputSchema: { type: 'object', properties: { name_or_prefix: { type: 'string' }, by: { type: 'string', description: '"name" (default) | "prefix"' } }, required: ['name_or_prefix'] } },
  ...
  {
    name: 'leaderboard',
    description: 'Fetch a Wynncraft leaderboard by type (e.g. \'combatGlobalLevel\', \'guildLevel\', \'warsCompletion\'); returns ranked entries up to resultLimit.',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const get = async (path: string, params?: URLSearchParams) => {
    ...
    if (!res.ok) throw await httpError(res, 'Wynncraft');
    return res.json();
  };
  switch (name) {
    case 'player':
    ...
    case 'item_search': {
      // Wynncraft 404s a search MISS instead of returning an empty list, so the
      // shared `get` helper turned every no-result query into an error. Combined
      // with the documented example being "Reincarnation" -- an item since removed
      // from the game -- the tool recorded zero successes in 7 days: models copied
      // example 1 and got a 404 every time. The upstream was never down.
      const query = reqStr(args, 'query', '"Idol"');
      const res = await pwFetch(`${BASE}/item/search/${encodeURIComponent(query)}`, {
        headers: { Accept: 'application/json', 'User-Agent': UA },
      });
      if (res.status === 404) return { query, count: 0, items: [] };
      if (!res.ok) throw await httpError(res, 'Wynncraft');
      return res.json();
    }
    case 'news':
